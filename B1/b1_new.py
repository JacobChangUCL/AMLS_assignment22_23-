...
model = FaceShapeRecognitionModel()
...
# 保存模型
torch.save(model.state_dict(), 'face_shape_recognition_model.pth')

if __name__ == "__main__":
    # 加载模型
    model = FaceShapeRecognitionModel()
    model.load_state_dict(torch.load('face_shape_recognition_model.pth'))
    model = model.to(device)
